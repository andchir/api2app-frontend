...
export class ApplicationSharedComponent implements OnInit, OnDestroy {
    ...
    fillDataFromQueryString(element: AppBlockElement): void {
        if (element.options?.outputApiFieldType === 'query_parameter' && element.options?.inputApiFieldName) {
            const value = this.route.snapshot.queryParamMap.get(String(element.options.inputApiFieldName));
            if (value !== null) {
                element.value = value;
            }
        }
