...
mode!=second
~~~
Также можно задавать конкретные значения, при которых будет виден элемент. Напрмиер, можно создать элемент "Выбор (select)" или "Радио-кнопки" с именем "select_mode", задать ему несколько значений.  

![Скиншот1](https://api2app.org/media/uploads/1772365457_visibility_1.jpg)

Потом создать другие элементы и в поле параметра "Скрыть в зависимости от поля" использовать такую запись:
~~~
select_mode==Первое значение
~~~

![Скиншот2](https://api2app.org/media/uploads/1772365468_visibility_2.jpg)
...
# Таблицы данных пользователей

В разделе "Данные" пользователь имеет возможность создавать и редиктировать таблицы базы данных, которые можно использовать при создании приложений.

Все таблицы имею API для упрадения данными. Для создания конфигураций API нужно нажать кнопку "Создать API". Конфигурации будут созданы в разделе "API".

Кроме стандартных имеются API агрегации:

```http
GET /api/v1/custom_tables/daily/sum/{string_field_name}/{numeric_field_name}
GET /api/v1/custom_tables/monthly/sum/{string_field_name}/{numeric_field_name}
GET /api/v1/custom_tables/daily/avg/{string_field_name}/{numeric_field_name}
GET /api/v1/custom_tables/monthly/avg/{string_field_name}/{numeric_field_name}
```

| Путь            | Что делает                                                   |
|-----------------|--------------------------------------------------------------|
| `/daily/sum/`   | Считает сумму числового поля по дням и группам               |
| `/daily/avg/`   | Считает среднее значение числового поля по дням и группам    |
| `/monthly/sum/` | Считает сумму числового поля по месяцам и группам            |
| `/monthly/avg/` | Считает среднее значение числового поля по месяцам и группам |

Например, `/daily/sum/status/total` вернет сумму `total` за каждый день отдельно для каждого значения поля `status`.

| Параметр             | Описание                                                    |
|----------------------|-------------------------------------------------------------|
| `string_field_name`  | Поле группировки. Допустимые типы: `string`, `text`, `date` |
| `numeric_field_name` | Числовое поле. Допустимые типы: `integer`, `float`          |

Query параметры:

| Параметр    | Описание                                                                  |
|-------------|---------------------------------------------------------------------------|
| `date_from` | Начало периода в формате `YYYY-MM-DD`; по умолчанию сегодня минус 30 дней |
| `date_to`   | Конец периода в формате `YYYY-MM-DD`; по умолчанию сегодня                |

Для агрегации в таблице должно быть хотя бы одно поле `date` или `datetime`. Первое такое поле по порядку используется как дата периода.

Пример:

```bash
curl "$CUSTOM_TABLES_URL/api/v1/custom_tables/daily/sum/status/total?date_from=2026-06-01&date_to=2026-06-30" \
